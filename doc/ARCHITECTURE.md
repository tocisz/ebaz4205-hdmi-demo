...
| Buzzer | TTC0 PWM | `beep -e /dev/input/event0` command |
...
> **Beeper** (`buzzer`, FPGA pin D18): The buzzer is driven by the **PS TTC0 PWM** (not GPIO), exposed through the PL as `ttc0_wave0_out`. The device tree declares a `pwm-beeper` at `pwms = <&ttc0 0 370370 0>` (2.7 kHz). The kernel driver stack is: `pwm-cadence.ko` (TTC PWM chip) → `pwm-beeper.ko` (input subsystem, `EV_SND`/`SND_TONE` events on `/dev/input/event0`). **The standard `beep` utility is BROKEN** on this platform because it opens `/dev/tty0` and uses the legacy `KIOCSOUND` ioctl — a PC‑speaker mechanism that is a no-op on ARM Zynq (the kernel's `kd_mksound()` is NULL). The **working invocation** is `beep -e /dev/input/event0 -f 2400`, which tells beep to write `EV_SND`/`SND_TONE` events directly to the evdev device instead of using `KIOCSOUND`. The TTC0 counter control register confirms correct operation: at run time `MATCH_MODE` and `INTR_MODE` are active; `WAVE_EN` (bit 5) is cleared by the `pwm-cadence` driver but the external output still functions correctly on this silicon revision. For convenience, consider `alias beep='beep -e /dev/input/event0'`.  
>  
...
Based on Xilinx U-Boot (`xlnx_rebase_v2025.01`), configured for the EBAZ4205 board. Key responsibilities:
- Loads the flattened device tree (`devicetree.dtb`, based on `zynq-ebaz4205.dts`)
- Loads the Linux kernel image (`uImage`)
- Sets kernel command line (`console=ttyPS1,115200 root=/dev/mmcblk0p2 ...`)
- **Serial0** (MIO 24-25, USB connector on expansion board): Linux console
- **Serial1** (EMIO, pin header on main board): U-Boot console for bootloader access without the expansion board
...
|---|---|---|
| p1 | FAT32 | `BOOT.bin` (FSBL + bitstream + U-Boot + DTB), `uImage`, `boot.scr`, `uboot.env` |
...
The LCD is declared as a child node of the PS SPI0 controller in `zynq-ebaz4205.dts`:
...
The kernel (build #3, 2026-07-15) has been updated with `CONFIG_OF_OVERLAY=y`,
`CONFIG_OF_CONFIGFS=y`, and `CONFIG_OF_FPGA_REGION=y`, enabling the full
FPGA Manager + Device Tree Overlay stack:
...
**Two limitations** currently prevent safe runtime PL reconfiguration:

1. **No PHY crystal** — the Ethernet PHY relies on the FPGA's `clk_25m` output
   (pin U18, from PS FCLK_CLK1). During PL reconfig, this clock stops, the
   PHY loses lock, and `eth0` goes down.
2. **PL peripheral nodes hardcoded in base DT** — mwipcore, LCD, GPIO EMIO,
   and beeper are in the base device tree, not in a reloadable overlay.

**Future directions** include adding a hardware 25 MHz oscillator (makes Ethernet
independent of PL) and implementing Partial Reconfiguration (swap HDMI design
while Ethernet, LCD, and GPIO continue running).
